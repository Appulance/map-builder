...
$(document).ready(async function () {
	...
	$("#aliases").change(function() {
		...
		for (var i = 0; i < markers.length; i++) {
			...
			current_date = markers[i].date;
			label_id = "#infobox-" + markers[i].serial + "-" + markers[i].alias;

			if (selected.includes(current) == true && selected_dates.includes(current_date) == true) {
				markers[i].setVisible(true);
				if (label_status) $(label_id).show();
	...
	$("#dates").change(function() {
		...
		ga('send', 'event', 'Dates', 'Date Change');

		for (var i = 0; i < markers.length; i++) {
			current = markers[i].date; 
			label_id = "#infobox-" + markers[i].serial + "-" + markers[i].alias;
			...
			if (selected.includes(current) == true) {
				...
				// reset bounds to group of related pins
				bounds.extend(markers[i].position);
				alias = markers[i].alias;
				serial = markers[i].serial;
				datetime = new Date(markers[i].serial);
				addTimelineEntryToTimeline(datetime, serial);
				addAliasEntryToAliases(alias);
			} else {
				markers[i].setVisible(false);
				if (label_status) $(label_id).hide();
			}
...
function addMarker(lat, lon, datetime, alias="Single", label = "", color = "blue") {
	...
	marker.addListener("click", () => {
		markerClickHandler(marker.serial, marker.date, marker.alias);
	});
...
function addMarkersToTimeline(arr) {
	$("#timeline").parent().prepend("<button id='timeline-all' type='button'>Select All Times</button>");
...
function addAliasEntryToAliases(alias, aliases = $("#aliases"), sel = true) {
	/*if ($("#aliases").val().some(el => el === alias)) {
		// we don't want duplicate aliases, so
		// if alias already exists, return
		return;
	} */
	
	if ($("#aliases").val().indexOf(alias) == -1) {
		let entry = new Option(alias, alias, sel, sel);
